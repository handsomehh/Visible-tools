...
function Jump() {
  ...
  if (content >= 0 && content < json_store.info.length - 1) {
    ...
    DrawP();
    document.getElementById("剩余数量").innerHTML =
      index + " / " + (json_store.info.length - 1);
    DisplayAll();
  }
}
  ...
  if (person_x) {
    ...
    for (let i = 0; i < json_store.labelData.length; i++) {
      if (json_store.labelData[i].label == "character attributes") {
        ...
        content +=
          "<br>人物序号:" +
          json_store.labelData[i].frames[0].attr["character number"];
        content +=
          "<br>性别:" + json_store.labelData[i].frames[0].attr["gender"];
        content += "<br>年龄:" + json_store.labelData[i].frames[0].attr["age"];
        content +=
          "<br>头发长度:" +
          json_store.labelData[i].frames[0].attr["hair length"];
        content +=
          "<br>是否带帽子:" +
          json_store.labelData[i].frames[0].attr["wear a hat"];
        content +=
          "<br>袖子长度:" +
          json_store.labelData[i].frames[0].attr["sleeve length"];
        content +=
          "<br>裤子类型" +
          json_store.labelData[i].frames[0].attr[
            "types of lower body clothing"
          ];
        content +=
          "<br>裤子长度" +
          json_store.labelData[i].frames[0].attr["bottom clothes lengthr"];
        content +=
          "<br>是否带眼镜:" +
          json_store.labelData[i].frames[0].attr["wear glasses"];
        content +=
          "<br>是否打领带:" +
          json_store.labelData[i].frames[0].attr["wear a tie"];
        content +=
          "<br>上衣种类:" +
          json_store.labelData[i].frames[0].attr[
    ...
    if (per_list.length > 0) {
      document.getElementById("人物属性零").innerHTML = per_list[0];
    }
    if (per_list.length > 1) {
      document.getElementById("人物属性一").innerHTML = per_list[1];
    }
    if (per_list.length > 2) {
      document.getElementById("人物属性二").innerHTML = per_list[2];
    }
    if (per_list.length > 3) {
      document.getElementById("人物属性三").innerHTML = per_list[3];
    }
  }
}
      ...
      if (json_store.labelData[i].label == "action") {
        for (j = 0; j < json_store.labelData[i].frames.length; j++) {
          if (json_store.labelData[i].frames[j].frame == index) {
            content +=
              "人物序号:" +
              json_store.labelData[i].frames[j].attr["character number"];
            content +=
              "<br>动作原语:" +
              json_store.labelData[i].frames[j].attr["action_verb"];
            content +=
              "<br>动作目标:" +
              json_store.labelData[i].frames[j].attr["action_noun"];
            content += "<br>";
          }
        }
      }
        ...
        for (j = 0; j < json_store.labelData[i].frames.length; j++) {
          if (json_store.labelData[i].frames[j].frame == index) {
            ...
            content +=
              ...
              json_store.labelData[i].frames[j].attr["character number"];
            content +=
              "<br>情绪:" + json_store.labelData[i].frames[j].attr["sentiment"];
            content += "<br>";
          }
        }
      }
        ...
        for (j = 0; j < json_store.labelData[i].frames.length; j++) {
          if (json_store.labelData[i].frames[j].frame == index) {
            ...
            content +=
              ...
              json_store.labelData[i].frames[j].attr["character number"];
            content +=
              "<br>场景" + json_store.labelData[i].frames[j].attr["scene"];
            content += "<br>";
          }
        }
      }
...
function DrawTect() {
  let arr = [];
  let flag = 0;
  for (let j = 0; j < json_store.labelData.length && j <= 6; j++) {
    //注：默认只看前六个
    if (json_store.labelData[j].label == "character attributes") {
      for (
        let i = 0;
        i < json_store.labelData[j].frames.length && i <= 2 * index;
        i++
      ) {
  ...
  if (flag) {
    for (let i = 0; i < arr.length; i++) {
      ...
      ctx.strokeStyle = "red";
      ctx.moveTo(
        (1011 / realWidth) * arr[i][0],
        (600 / realHeight) * arr[i][1]
      );
      ctx.lineTo(
        (1011 / realWidth) * arr[i][0],
        (600 / realHeight) * arr[i][3]
      );
      ctx.lineTo(
        (1011 / realWidth) * arr[i][2],
        (600 / realHeight) * arr[i][3]
      );
      ctx.lineTo(
        (1011 / realWidth) * arr[i][2],
